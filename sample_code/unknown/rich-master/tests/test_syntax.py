...
def test_blank_lines():
    code = "\n\nimport this\n\n"
    syntax = Syntax(
        code, lexer="sample_code", theme="ascii_light", code_width=30, line_numbers=True
    )
...
def test_get_number_styles():
    syntax = Syntax(CODE, "sample_code", theme="monokai", line_numbers=True)
...
def test_syntax_guess_lexer():
    assert Syntax.guess_lexer("banana.py") == "sample_code"
    assert Syntax.guess_lexer("banana.py", "import this") == "sample_code"
...
def test_syntax_padding():
    syntax = Syntax("x = 1", lexer="sample_code", padding=(1, 3))
    console = Console(
        width=20,
        file=io.StringIO(),
...
def test_syntax_measure():
    console = Console()
    code = Syntax("Hello, World", "sample_code")
    assert code.__rich_measure__(console, console.options) == Measurement(0, 12)

    code = Syntax("Hello, World", "sample_code", line_numbers=True)
    assert code.__rich_measure__(console, console.options) == Measurement(3, 16)

    code = Syntax("Hello, World", "sample_code", code_width=20, line_numbers=True)
    assert code.__rich_measure__(console, console.options) == Measurement(3, 24)

    code = Syntax("", "sample_code", code_width=20, line_numbers=True)
    assert code.__rich_measure__(console, console.options) == Measurement(3, 24)
