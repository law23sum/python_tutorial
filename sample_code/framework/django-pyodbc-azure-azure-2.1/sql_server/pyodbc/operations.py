...
class DatabaseOperations(BaseDatabaseOperations):
    ...
    def date_trunc_sql(self, lookup_type, field_name):
        ...
        if lookup_type == 'quarter':
            return "CONVERT(datetime2, CONVERT(varchar, DATEPART(year, %s)) + '/' + CONVERT(varchar, 1+((DATEPART(quarter, %s)-1)*3)) + '/01')" % (
            field_name, field_name)
        if lookup_type == 'month':
            return "CONVERT(datetime2, CONVERT(varchar, DATEPART(year, %s)) + '/' + CONVERT(varchar, DATEPART(month, %s)) + '/01')" % (
            field_name, field_name)
        if lookup_type == 'week':
            return "DATEADD(DAY, (DATEPART(weekday, %s) + 5) %%%% 7 * -1, CONVERT(datetime2, CONVERT(varchar(12), %s, 112)))" % (
            field_name, field_name)
        if lookup_type == 'day':
