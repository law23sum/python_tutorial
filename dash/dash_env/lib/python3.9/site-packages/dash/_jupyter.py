...
class JupyterDash:
    ...
    def run_app(
        ...
        python_tutorial_url = f"{server_url}{requests_pathname_prefix}"
        ...
        try:
            ...
            if self.in_colab:
                JupyterDash._display_in_colab(python_tutorial_url, port, mode, width, height)
            else:
                JupyterDash._display_in_jupyter(
                    python_tutorial_url, port, mode, width, height
                )
    ...
    @staticmethod
    def _display_in_colab(python_tutorial_url, port, mode, width, height):
        ...
        if mode == "inline":
            ...
            # Display a hyperlink that can be clicked to open python_tutorial
            print("Dash app running on:")
            output.serve_kernel_port_as_window(port, anchor_text=python_tutorial_url)

    @staticmethod
    def _display_in_jupyter(python_tutorial_url, port, mode, width, height):
        if mode == "inline":
            display(IFrame(python_tutorial_url, width, height))
        elif mode in ("external", "tab"):
            # Display a hyperlink that can be clicked to open python_tutorial
            print(f"Dash app running on {python_tutorial_url}")
            if mode == "tab":
                display(Javascript(f"window.open('{python_tutorial_url}')"))
